...
class GameSurface(ScreenHandle):
    ...
    def left_corner(self):
        ...
        width_center = self.get_width() // sprite_size // 2
        height_center = self.get_height() // sprite_size // 2 
        hero_position = self.engine.hero.position

        if hero_position[0] > width_center:
            min_x = hero_position[0] - width_center

        if hero_position[1] > height_center:
            min_y = hero_position[1] - height_center
...
class ProgressBar(ScreenHandle):
    ...
    def draw(self, canvas):
        ...
        self.blit(font.render(
                  ...
                  colors["black"]), (60, 30))
        self.blit(font.render(
                  f'{self.engine.hero.exp}/{(100*(2**(self.engine.hero.level-1)))}', True, colors["black"]), (60, 70))
        self.blit(font.render(f'Level', True, colors["black"]), (270, 30))
        self.blit(font.render(f'Gold', True, colors["black"]), (270, 70))
        self.blit(font.render(f'{self.engine.hero.level}', True, 
                  colors["black"]), (330, 30))
        self.blit(font.render(f'{self.engine.hero.gold}', True, 
                  colors["black"]), (330, 70))
        self.blit(font.render(f'Strength', True, colors["black"]), (380, 30))
        self.blit(font.render(f'Luck', True, colors["black"]), (380, 70))
